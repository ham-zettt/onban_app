...
@section('content')
    {{-- buatkan form editnya --}}
    <div class="w-full  my-6 pr-0 lg:pr-2">
        <div class="leading-loose">
            <form class="p-10 bg-white rounded shadow-xl" method="post"
                action="{{ route('vouchers.update', $voucher->kode_voucher) }}">
                @csrf
                @method('PUT')
                <div class="">
                    ...
                    <input class="w-full px-5 py-1 text-gray-700 bg-gray-50 rounded" id="kode_voucher" name="kode_voucher"
                        type="text" placeholder="Kode Voucher" value="{{ $voucher->kode_voucher }}" />
                    @error('kode_voucher')
                ...
                <div class="mt-2">
                    ...
                    <input class="w-full px-5 py-1 text-gray-700 bg-gray-50 rounded" id="nama_voucher" name="nama_voucher"
                        type="text" placeholder="Nama Voucher" value="{{ $voucher->nama_voucher }}" />
                    @error('nama_voucher')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mt-2 max-w-2xl">
                    <label for="potongan_harga"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Potongan
                        Harga Voucher</label>
                    <select id="potongan_harga" name="potongan_harga"
                        ...
                        @foreach ($potonganHarga as $label => $nilai)
                            <option value="{{ $nilai }}" {{ $voucher->$label == $nilai ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-2">
                    ...
                    <input type="date" class=" text-gray-700 bg-gray-50 rounded" name="tanggal_mulai" id="tanggal_mulai"
                ...
                <div class="mt-2">
                    ...
                    <input type="date" class=" text-gray-700 bg-gray-50 rounded" name="tanggal_berakhir"
                        id="tanggal_berakhir" value="{{ $voucher->tanggal_berakhir }}">
